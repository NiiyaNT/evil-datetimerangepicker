/**
* @author: Niiyant https://github.com/NiiyaNT
* @copyright: Copyright (c) 2024 NiiyaNT. All rights reserved.
* @license: Licensed under the MIT license. See http://www.opensource.org/licenses/mit-license.php
*
* Special thanks to Alumuko and Dan Grossman.
* This program is base on Alumuko vanilla-daterage-picker(0.2.0) and Dan Grossman's bootstrap-daterangepicker (version 3.1)
*/
...
(function () {
    DateRangePicker = function (element, options, cb) {
        ...
        this.ranges = {};
        this.timePicker24HourCheckbox = false;
        ...
        this.locale = {
            ...
            timePicker24HourCheck: 'Enable time selector'
        ...
        if (typeof options.locale === 'object') {
            ...
            if (typeof options.locale.timePicker24HourCheck === 'string') 
              this.locale.timePicker24HourCheck = options.locale.timePicker24HourCheck
        ...
        if (typeof options.timePicker24HourCheckbox === 'boolean')
            this.timePicker24HourCheckbox = options.timePicker24HourCheckbox;
        ...
        */
        ...
        this.updateElement();
        if (this.timePicker24HourCheckbox) {
            var rangesContainer = this.container.querySelector('.ranges');
            var checkboxDiv = document.createElement('div');
            checkboxDiv.className = 'timepicker-toggle';
            checkboxDiv.style.padding = '10px 15px';
            checkboxDiv.innerHTML = '<label><input type="checkbox" id="toggleTimePicker">' + this.locale.timePicker24HourCheck +'</label>';
            rangesContainer.appendChild(checkboxDiv);

            var toggleTimePicker = document.getElementById('toggleTimePicker');
            self = this;
            toggleTimePicker.addEventListener('change', function () {
                var enableTimePicker = toggleTimePicker.checked;
                if (enableTimePicker) {
                    self.timePicker = true;
                    self.updateView();
                    console.log("awa")
                } else {
    ...
    DateRangePicker.prototype = {
        ...
        removeTimePicker: function () {
            // Oculta el time picker en lugar de eliminar el contenido completamente
            var timePickerElementLeft = this.container.querySelector('.drp-calendar.left .calendar-time');
            var timePickerElementRight = this.container.querySelector('.drp-calendar.right .calendar-time');
            if (timePickerElementLeft) {
                timePickerElementLeft.innerHTML = '';
            }
            if (timePickerElementRight) {
                timePickerElementRight.innerHTML = '';
            }


            // Configurar la hora de inicio a 00:00:00 y la hora de finalización a 23:59:59
